...
export function BankStatementUpload({ 
  ...
  const { processBankCsv } = useUploadManager()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const extractedTransactions = await processBankCsv(bankCsvFile)
      ...
      setError(`Erro ao processar CSV bancário: ${error.message}`)
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button type="submit" disabled={!bankCsvFile}>
