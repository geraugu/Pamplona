...
export default function UploadPage() {
  ...
  const handleCreditCardExtract = async (extractedTransactions: Transacao[]) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao extrair transações do cartão de crédito')
    }
  }
  ...
  const handleInvestmentUpload = async (csvFile: File, investmentDate: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao processar arquivo de investimentos')
    }
  }
  ...
  const safeFindMatchingCategory = (description: string) => {
    const result = findMatchingCategory(description, categoryMapping, transactionsReferencia)
    return {
